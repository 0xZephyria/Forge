...
pub const ForgeVM = struct {
    ...
    // -------------------------------------------------------------------
    // Custom (ZEPH) instruction dispatch — LEGACY PATH
    // -------------------------------------------------------------------
    // NOTE: The Forge compiler no longer emits custom-0..3 opcodes.
    // All syscalls are now issued as: ADDI a0, zero, <syscall_id> ; ECALL().
    // The ECALL goes through execSystem → syscallHandler directly.
    //
    // This execCustom path is kept for:
    //   1. Hand-crafted bytecode that may use the old encoding
    //   2. Future use if we re-enable direct custom opcodes
    //
    // The opcode values here MUST match riscv.zig ZephCustomOp and
    // dispatch.zig SyscallId for consistency.
    ...
    pub fn execCustom(self: *ForgeVM, c: decoder.CustomType) void {
        ...
        // Map ZephCustomOp (from riscv.zig) directly to SyscallId (from dispatch.zig).
        // The custom op already encodes the syscall ID — we just need to set a0
        // to the syscall ID and call the handler.
        const syscallId: u32 = switch (op) {
            // Storage (0x01-0x06)
            0x01 => 0x01, // STATE_READ
            0x02 => 0x02, // STATE_WRITE
            0x03 => 0x03, // STATE_READ_DERIVED
            0x04 => 0x04, // STATE_WRITE_DERIVED
            0x05 => 0x05, // STATE_READ_GLOBAL
            0x06 => 0x06, // STATE_WRITE_GLOBAL
            // Assets (0x10-0x16)
            0x10 => 0x10, // ASSET_TRANSFER
            0x11 => 0x11, // ASSET_BALANCE
            0x12 => 0x12, // ASSET_MINT
            0x13 => 0x13, // ASSET_BURN
            0x17 => 0x17, // NATIVE_TRANSFER
            // Authority (0x20-0x22)
            0x20 => 0x20, // AUTH_CHECK
            0x21 => 0x21, // AUTH_GRANT
            0x22 => 0x22, // AUTH_REVOKE
            // Events (0x30-0x31)
            0x30 => 0x30, // EMIT_EVENT
            0x31 => 0x31, // EMIT_INDEXED_EVENT
            // Cross-contract (0x40)
            0x40 => 0x40, // CALL_CONTRACT
            0x45 => 0x45, // SCHEDULE_CALL
            // Execution control (0x51)
            0x51 => 0x51, // REVERT
            // Environment (0x60-0x6C)
            0x60 => 0x60, // GET_CALLER
            0x61 => 0x61, // GET_VALUE
            0x64 => 0x64, // GET_THIS
            0x65 => 0x65, // GET_BLOCK
            0x66 => 0x66, // GET_NOW
            0x68 => 0x68, // GET_GAS
            0x6C => 0x6C, // VRF_RANDOM
            // Extended (0xA0+)
            0xA0 => 0xA0, // ORACLE_QUERY
            0xB0 => 0xB0, // ZK_VERIFY
            0xB1 => 0xB1, // DELEGATE_GAS
            0xB2 => 0xB2, // EXPAND_ACCOUNT
            0xB3 => 0xB3, // CLOSE_ACCOUNT
            // Debug
            0xFF => 0xFF, // LOG_DIAGNOSTIC
            else => {
                self.status = .fault;
