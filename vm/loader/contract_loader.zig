...
/// C-calling-convention wrapper around BLAKE3 hashing for the AOT inline path.
fn hashBlake3C(input: [*]const u8, input_len: u32, output: [*]u8) callconv(.c) void {
    std.crypto.hash.Blake3.hash(input[0..input_len], output[0..32], .{});
}

/// C-calling-convention wrapper for BLS verification on the AOT inline path.
/// Respects the node-provided blsVerifyFn override if set; otherwise uses the
/// native bls12_381 library.
/// Returns 1 if valid, 0 if invalid.
fn blsVerifyWrapper(vm_opaque: ?*anyopaque, pk: [*]const u8, sig: [*]const u8, msg: [*]const u8, msg_len: u32) callconv(.c) i32 {
    const vm: *executor.ForgeVM = @ptrCast(@alignCast(vm_opaque orelse return 0));
    const env: ?*syscallDispatch.HostEnv = @ptrCast(@alignCast(vm.hostCtx));

    // Check node provider override first.
    if (env) |e| {
        if (e.blsVerifyFn) |verify| {
            const pk_array: [48]u8 = @as(*const [48]u8, @ptrCast(pk)).*;
            const sig_array: [96]u8 = @as(*const [96]u8, @ptrCast(sig)).*;
            return if (verify(pk_array, sig_array, msg[0..msg_len])) 1 else 0;
        }
    }

    // Native BLS verification via bls12_381 library.
    const public_key = bls.PublicKey.fromBytes(pk[0..48]) catch return 0;
    const signature = bls.Signature.fromBytes(sig[0..96]) catch return 0;
    const dst = "FORGE_BLS_SIG_V1";
    signature.verify(true, msg[0..msg_len], dst, null, &public_key, true) catch return 0;
    return 1;
}

/// C-calling-convention wrapper for appending log entries from the AOT inline path.
/// Reads topics and data from VM memory via the ForgeVM pointer embedded in vm_ctx.
/// Returns 0 on success, non-zero on failure.
fn appendLogC(vm_opaque: ?*anyopaque, topic_count: u32, topics_ptr: u64, data_ptr: u64, data_len: u64) callconv(.c) i32 {
    const vm: *executor.ForgeVM = @ptrCast(@alignCast(vm_opaque orelse return 5));
    const env: *syscallDispatch.HostEnv = @ptrCast(@alignCast(vm.hostCtx orelse return 5));

    var logEntry = syscallDispatch.LogEntry.init(env.allocator);
    errdefer logEntry.deinit();

    var i: u32 = 0;
    while (i < topic_count) : (i += 1) {
        const topic_offset = topics_ptr + @as(u64, i) * 32;
        const slice = vm.memory.getSlice(topic_offset, 32) catch return 5;
        var topic: [32]u8 = undefined;
        @memcpy(&topic, slice);
        logEntry.topics.append(logEntry.alloc, topic) catch return 5;
    }

    if (data_len > 0) {
        const data_slice = vm.memory.getSlice(data_ptr, @intCast(data_len)) catch return 5;
        logEntry.data.appendSlice(logEntry.alloc, data_slice) catch return 5;
    }

    env.logs.append(env.allocator, logEntry) catch return 5;
    return 0;
}
...
fn compileAndRunAot(
    ...
    const ctx = aot.AotContext{
        ...
        .bloom_filter = &env.bloom_filter,
        .hash_blake3 = hashBlake3C,
        .bls_verify_fn = blsVerifyWrapper,
        .append_log = appendLogC,
    };
