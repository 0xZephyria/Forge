...
//
// Access lists are not embedded in the binary — the DAG-based mempool
// resolves conflicts implicitly at the blockchain consensus layer.
//
...
pub const ZephBinHeader = extern struct {
    ...
    actionCount: u16,
    _pad0: u16,
    _reserved: u32, // was accessListLen; DAG mempool makes it implicit
    bytecodeLen: u32,
    checksum: u32,
    dataSectionLen: u32,
    conservation_len: u32,
};
comptime {
...
pub fn parse(allocator: std.mem.Allocator, data: []const u8) ParseError!ZephBinPackage {
    ...
    // Locate sections (Forge compiler layout: Data section, then Bytecode)
    const dsStart: usize = @sizeOf(ZephBinHeader);
    const bcStart: usize = dsStart + hdr.dataSectionLen;
...
test "parse empty contract (0 actions)" {
    ...
    @memcpy(data[0..4], "FORG");
    std.mem.writeInt(u16, data[4..6], ZEPHBIN_VERSION, .little);
    // bytecodeLen = 2 (just the u16 actionCount=0), dataSectionLen = 0
