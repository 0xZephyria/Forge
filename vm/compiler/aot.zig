...
pub const AotContext = extern struct {
    ...
    // Inline syscall helpers — added at end so existing cached AOT .dylibs (which
    // only know the old field offsets) don't crash; they simply ignore these.
    bloom_filter: [*]u8,
    hash_blake3: *const fn (input: [*]const u8, input_len: u32, output: [*]u8) callconv(.c) void,
    bls_verify_fn: ?*const fn (vm_ctx: ?*anyopaque, pk: [*]const u8, sig: [*]const u8, msg: [*]const u8, msg_len: u32) callconv(.c) i32,
    append_log: ?*const fn (vm_ctx: ?*anyopaque, topic_count: u32, topics_ptr: u64, data_ptr: u64, data_len: u64) callconv(.c) i32,
};
...
pub fn generateCSource(allocator: std.mem.Allocator, pkg: *const zephbinLoader.ZephBinPackage) ![]const u8 {
    ...
    try writer.writeAll(
        ...
        \\    void (*hash_blake3)(const uint8_t* input, uint32_t input_len, uint8_t* output);
        \\    int32_t (*bls_verify_fn)(void* vm_ctx, const uint8_t* pk, const uint8_t* sig, const uint8_t* msg, uint32_t msg_len);
        \\    int32_t (*append_log)(void* vm_ctx, uint32_t topic_count, uint64_t topics_ptr, uint64_t data_ptr, uint64_t data_len);
        \\} AotContext;
        \\
...
fn translateInstruction(writer: anytype, insn: decoder.Instruction, pc: u32, stubOffset: u32, code: []const u8) !void {
    ...
    switch (insn) {
        ...
        .system => |sys| {
            switch (sys) {
                .ecall => {
                    ...
                    // Detect syscall ID from preceding ADDI x10, x0, <imm>.
                    // The compiler always emits ADDI+ECALL, so the syscall ID is known
                    // at transpile time.  For hot syscalls we generate inline C to
                    // avoid the C→Zig syscall_handler bridge.
                    const maybe_syscall_id = if (pc >= 4) blk: {
                        const prev_word = std.mem.readInt(u32, code[pc - 4 ..][0..4], .little);
                        const opcode: u7 = @truncate(prev_word & 0x7F);
                        if (opcode != 0x13) break :blk null;
                        const rd: u5 = @truncate((prev_word >> 7) & 0x1F);
                        if (rd != 10) break :blk null;
                        const funct3: u3 = @truncate((prev_word >> 12) & 0x7);
                        if (funct3 != 0) break :blk null;
                        const rs1: u5 = @truncate((prev_word >> 15) & 0x1F);
                        if (rs1 != 0) break :blk null;
                        break :blk @as(u32, @truncate(prev_word >> 20));
                    } else null;

                    if (maybe_syscall_id) |sid| switch (sid) {
                        0x31 => {
                            try writer.print(
                                \\        {{
                                \\            uint64_t topicCount = ctx->regs[11];
                                \\            uint64_t topicsPtr = ctx->regs[12];
                                \\            uint64_t dataPtr = ctx->regs[13];
                                \\            uint64_t dataLen = ctx->regs[14];
                                \\            if (topicCount > 4) {{ *ctx->status = 4; goto end; }}
                                \\            gas_used += 100 + 50 * topicCount + 4 * dataLen + 100;
                                \\            if (gas_used > gas_limit) {{ *ctx->gas_used = gas_limit; *ctx->status = 3; goto end; }}
                                \\            // Bloom filter: hash each topic with BLAKE3 -> 4 bit positions
                                \\            for (uint32_t i = 0; i < topicCount; i++) {{
                                \\                uint8_t topic[32];
                                \\                for (uint32_t j = 0; j < 32; j++) {{
                                \\                    uint64_t addr = topicsPtr + i * 32 + j;
                                \\                    if (addr >= ctx->memory_size) {{ *ctx->status = 4; goto end; }}
                                \\                    topic[j] = ctx->memory_backing[addr];
                                \\                }}
                                \\                uint8_t hash[32];
                                \\                ctx->hash_blake3(topic, 32, hash);
                                \\                for (int k = 0; k < 4; k++) {{
                                \\                    uint64_t window = *(uint64_t*)(hash + k * 8);
                                \\                    uint16_t bit_pos = (uint16_t)(window & 2047);
                                \\                    uint8_t byte_idx = (uint8_t)(bit_pos >> 3);
                                \\                    uint8_t bit_idx = (uint8_t)(bit_pos & 7);
                                \\                    ctx->bloom_filter[255 - byte_idx] |= (uint8_t)(1u << bit_idx);
                                \\                }}
                                \\            }}
                                \\            ctx->regs[0] = 0;
                                \\            if (ctx->append_log) {{
                                \\                int32_t log_err = ctx->append_log(ctx->vm_ctx, (uint32_t)topicCount, topicsPtr, dataPtr, dataLen);
                                \\                if (log_err != 0) {{ *ctx->status = 4; goto end; }}
                                \\            }}
                                \\        }}
                                \\        pc = {d};
                                \\
                        ...
                        0x73 => {
                            try writer.print(
                                \\        {{
                                \\            uint64_t pubkeyPtr = ctx->regs[11];
                                \\            uint64_t sigPtr = ctx->regs[12];
                                \\            uint64_t msgPtr = ctx->regs[13];
                                \\            uint64_t msgLen = ctx->regs[14];
                                \\            if (pubkeyPtr + 48 > ctx->memory_size || sigPtr + 96 > ctx->memory_size || msgPtr + msgLen > ctx->memory_size) {{ *ctx->status = 4; goto end; }}
                                \\            gas_used += 45000;
                                \\            if (gas_used > gas_limit) {{ *ctx->gas_used = gas_limit; *ctx->status = 3; goto end; }}
                                \\            uint8_t pk[48]; memcpy(pk, ctx->memory_backing + pubkeyPtr, 48);
                                \\            uint8_t sig[96]; memcpy(sig, ctx->memory_backing + sigPtr, 96);
                                \\            ctx->regs[0] = 0;
                                \\            if (ctx->bls_verify_fn) {{
                                \\                ctx->regs[10] = (uint64_t)ctx->bls_verify_fn(ctx->vm_ctx, pk, sig, ctx->memory_backing + msgPtr, (uint32_t)msgLen);
                                \\            }} else {{
                                \\                *ctx->gas_used = gas_used;
                                \\                sys_err = ctx->syscall_handler(ctx->vm_ctx, 0x73);
                                \\                ctx->regs[0] = 0;
        ...
        .custom => |c| {
            ...
            // Direct 1:1 mapping: op value = SyscallId = ZephCustomOp value.
            // Matches execCustom in executor.zig and riscv.zig ZephCustomOp enum.
            // The compiler no longer emits custom-0..3 opcodes (uses ADDI+ECALL),
            // but this legacy path must agree with the interpreter for correctness.
            if (op >= 0x01 and op <= 0x06) {
                // Storage ops: a1 = key ptr, a2 = result/value ptr
                try writer.writeAll(
                    \\            ctx->regs[11] = origA1;
                    \\            ctx->regs[12] = origA2;
                    \\
                ...
                // ASSET_TRANSFER: a1 = asset_id, a2 = from, a3 = to, a4 = amount
                try writer.writeAll(
                    \\            ctx->regs[11] = origA1;
                    \\            ctx->regs[12] = origA2;
                    \\            ctx->regs[13] = origA3;
            ...
            } else if (op >= 0x20 and op <= 0x22) {
                // Authority: a1 = account/role
                try writer.writeAll(
                    \\            ctx->regs[11] = origA1;
                    \\
                );
            } else if (op >= 0x30 and op <= 0x31) {
                // Events: a1 = event data, a2 = data, a3 = len
                try writer.writeAll(
                    \\            ctx->regs[11] = origA1;
                    \\            ctx->regs[12] = origA2;
                    \\            ctx->regs[13] = origA3;
                ...
                // CALL_CONTRACT: a1 = target, a2 = selector, a3 = data, a4 = len
                try writer.writeAll(
                    \\            ctx->regs[11] = origA1;
                    \\            ctx->regs[12] = origA2;
                    \\            ctx->regs[13] = origA3;
                ...
                // SCHEDULE_CALL: a1 = target, a2 = delay, a3 = data, a4 = len
                try writer.writeAll(
                    \\            ctx->regs[11] = origA1;
                    \\            ctx->regs[12] = origA2;
                    \\            ctx->regs[13] = origA3;
            ...
            try writer.print("            syscallId = 0x{X:0>2};\n", .{op});
